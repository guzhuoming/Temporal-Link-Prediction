...
for node in node_set:
    file = open('E:\\ht\\data-only-1-hop-without0-639668\\' + node + '.csv')
    print(node)
    # 对源节点进行清洗
    # 1.from或者to为null时，contractAddress不为null，好像交易额都是0，我们不考虑这种，将其删去
    # 2.将isError=1的删除
    df = pd.read_csv(file)
    df.drop(['Unnamed: 0', 'BlockHeight', 'ContractAddress', 'Input'], axis = 1, inplace=True)
    df = df[df['isError']==0]
    ...
    while i<len(df):
        if df['To'][i] == node and df['From'][i] not in set_1hop:
            set_1hop.append(df['From'][i])
            num += 1
        if df['From'][i] == node and df['To'][i] not in set_1hop:
    ...
    df_set_1_hop_name.to_csv('E:\\exchange_0x3f5ce5fbfe3e9af3971dd833d26ba9b5c936f0be_12\\1_hop_name.csv', index=False)
    ...
    for i in range(len(set_1hop)):
        ...
        df_0_1.to_csv('E:\\exchange_0x3f5ce5fbfe3e9af3971dd833d26ba9b5c936f0be_12\\0_1\\' + node + '_' + set_1hop[i] + '.csv')
        csvwriter.writerow([num, node + '_' + set_1hop[i]])
        num+=1
        print(num)

# --------------------------后面是之前的代码别管

# #----- 以0-1阶挑选的90个节点对的一阶节点爬取二阶节点
# import pandas as pd
# import csv
# import os
...
#     df_0_1.index = range(len(df_0_1))#对序号重新排序
#
#     df_0_1.to_csv('E:\\exchange_0x3f5ce5fbfe3e9af3971dd833d26ba9b5c936f0be_12\\0_1\\' + '0x3f5ce5fbfe3e9af3971dd833d26ba9b5c936f0be' + '_' + set_1hop[i] + '.csv')
...
#
#         df_1_2.to_csv('E:\\exchange_0x3f5ce5fbfe3e9af3971dd833d26ba9b5c936f0be_12\\1_2\\'+set_1hop[i]+'_'+set_2_hop[j]+'.csv')
