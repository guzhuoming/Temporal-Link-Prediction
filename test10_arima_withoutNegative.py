...
for i in range(len(name_node_pairs)):
    ...
    for i in range(len(df_2['prediction_ARIMA'])):
        if df_2['prediction_ARIMA'][i]<0:
            df_2['prediction_ARIMA'][i] = 0
    # print(df_2['tran_sum_real'].values)
    # print(df_2['prediction_ARIMA'].values)
    # print(df_2['tran_sum_real'].values - df_2['prediction_ARIMA'].values)
    rms = (np.mean(np.power((df_2['tran_sum_real'].values - df_2['prediction_ARIMA'].values), 2)))
    # print(rms)
    mse = mse+rms
    print(mse)
