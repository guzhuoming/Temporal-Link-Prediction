# 本代码将以0x3f为源节点挑选的节点对，以7天为间隔，挑选出拥有连续5个时间段交易的12个节点对。
import pandas as pd
import csv
import numpy as np
import math
...
#将挑选出的交易记录保存在一个文件夹里
for i in range(len(name_node_pairs_selected)):
    print(i)
    print(name_node_pairs_selected[i])
    file = open('E:\\exchange_0x3f5ce5fbfe3e9af3971dd833d26ba9b5c936f0be_90\\0_1\\'+name_node_pairs_selected[i]+'.csv')
    df = pd.read_csv(file)
    df.to_csv('E:\\exchange_0x3f5ce5fbfe3e9af3971dd833d26ba9b5c936f0be_90\\node_pairs_selected_7days\\'+name_node_pairs_selected[i]+'.csv', index=False)
# 只保留这些节点在95-99时的值
...
for i in range(len(name_node_pairs_selected)):
    ...
    df.to_csv('E:\\exchange_0x3f5ce5fbfe3e9af3971dd833d26ba9b5c936f0be_90\\node_pairs_selected_5_7days\\' + name_node_pairs_selected[i] + '.csv', index=False)
